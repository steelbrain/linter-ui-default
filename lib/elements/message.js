    ...
    return (<linter-message class={message.severity}>
      { message.description && (
        <a href="#" onClick={() => this.toggleDescription()}>
          <span className={`icon linter-icon icon-${this.state.descriptionShow ? 'chevron-up' : 'chevron-right'}`}></span>
        </a>
      )}
      <linter-excerpt>
      ...
      { this.state.descriptionShow && (
        <div dangerouslySetInnerHTML={{ __html: this.state.description || 'Loading...' }}></div>
      ) }
    </linter-message>)
  }
}
