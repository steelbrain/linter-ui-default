...
import {getMessageRange, getMessageClass} from '../helpers'
...
export class MessageElement {
  ...
  attach(message, showLink) {
    ...
    if (message.filePath && showLink) {
      ...
      let text
      if (messageRange) {
        text = `at line ${messageRange.start.row + 1} col ${messageRange.start.column + 1} in ${filePath}`
      } else {
        text = `in ${filePath}`
      }

      this.elementLink.textContent = text
      this.linkCallback = MessageElement.attachLink(this.elementLink, message.filePath, messageRange ? messageRange.start : null)
    } else {
      this.elementLink.textContent = ''
    }

    this.elementType.textContent = message.type
    this.elementType.className = `linter-message-item badge badge-flexible linter-highlight ${getMessageClass(message)}`
