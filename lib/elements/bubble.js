...
export function getElement(messages: Set<Linter$Message>, includeProvider: boolean): HTMLElement {
  ...
  for (const message of messages) {
    children.push(<Message message={message} includeLink={false} includeCopyLink={false} includeProvider={includeProvider} />)
    if (message.trace && message.trace.length) {
      for (const trace of message.trace) {
        children.push(<Message message={trace} includeLink={true} includeCopyLink={false} includeProvider={includeProvider} />)
      }
    }
  }
