...
type Messages$State = { messages: Array<Message>, showProviderName: boolean, showCopyButton: boolean }
  ...
  render() {
    ...
    return (<linter-messages style={messageStyle} tabindex="-1" onKeyDown={Messages.onKeyDown}>
      { sortMessages(this.state.messages).map(message =>
        <MessageElement message={message} key={message.key} includeLink={true} includeCopyButton={this.state.showCopyButton} includeProvider={this.state.showProviderName} />
      ) }
    </linter-messages>)
  }
