  ...
  render() {
    ...
    return (<linter-messages style={messageStyle} tabindex="-1" onKeyDown={Messages.onKeyDown}>
      { sortMessages(this.state.messages).map(message =>
        message.version === 1 ?
          <MessageElementLegacy reference="internal" message={message} showProviderName={this.state.showProviderName} key={message.key} /> :
          <MessageElement reference="internal" message={message} showProviderName={this.state.showProviderName} key={message.key} />
      ) }
    </linter-messages>)
  }
