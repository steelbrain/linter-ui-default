...
// TODO: Store the height in config
export default class PanelComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.delegate.onDidChangePanelConfig((panelHeight) => {
      console.log('updating state to make it re-render')
  ...
  onResizeStop = (direction: 'top', size: { width: number, height: number }) => {
    console.log('updating height to', size.height)
  ...
  render() {
    ...
    if (this.state.tempHeight) {
      ...
      if (this.props.delegate.panelTakeMinimumHeight) {
        height = 'auto'
        customStyle = {
          maxHeight: this.props.delegate.panelHeight,
        }
      } else {
    ...
    return (
      <ResizableBox isResizable={{ top: true }} onResize={this.onResize} onResizeStop={this.onResizeStop} height={height} customStyle={customStyle}>
