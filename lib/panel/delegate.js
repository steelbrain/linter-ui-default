...
export default class PanelDelegate {
  ...
  panelTakeMinimumHeight: boolean;
  ...
  constructor() {
    ...
    this.subscriptions.add(atom.config.observe('linter-ui-default.panelTakeMinimumHeight', (panelTakeMinimumHeight) => {
      const notInitial = typeof this.panelTakeMinimumHeight !== 'undefined'
      this.panelTakeMinimumHeight = panelTakeMinimumHeight
      if (notInitial) {
        this.emitter.emit('observe-panel-config')
      }
    }))
