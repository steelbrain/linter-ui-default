...
export default class Editor {
  ...
  updateTooltip(position: ?Point) {
    ...
    if (this.tooltip && this.tooltip.messages.length === 1 && this.messages.has(this.tooltip.messages[0]) && pointInMessageRange(position, this.tooltip.messages[0])) {
      return
    }
    this.removeTooltip()
    if (!this.showTooltip) {
    ...
    const tooltip = this.tooltip = new Tooltip(messages, position, this.textEditor)
    tooltip.onDidDestroy(() => {
      this.tooltip = null
    })
  }
  removeTooltip() {
