...
export default class Editor {
  ...
  listenForCurrentLine() {
    this.subscriptions.add(this.textEditor.observeCursors((cursor) => {
      let marker
      let lastLine = -1
      const handlePositionChange = (bufferPosition) => {
        const gutter = this.gutter
        const currentLine = bufferPosition.row
        if (currentLine === lastLine) return
        if (marker) marker.destroy()
        if (!gutter) return
        // NOTE: This is to not-show during selections
        if (!cursor.getMarker().getBufferRange().isEmpty()) return

        lastLine = currentLine
        marker = this.textEditor.markBufferRange([[currentLine, 0], [currentLine, Infinity]], {
          invalidate: 'never',
          maintainHistory: false,
        })
        const item = document.createElement('span')
        item.className = 'line-number cursor-line cursor-line-no-selection linter-cursor-line'
      ...
      subscriptions.add(cursor.onDidChangePosition(({ newBufferPosition }) => {
        handlePositionChange(newBufferPosition)
      }))
      subscriptions.add(cursor.onDidDestroy(() => {
        this.subscriptions.delete(subscriptions)
      }))
      this.subscriptions.add(subscriptions)
      handlePositionChange(cursor.getBufferPosition())
    }))
  }
