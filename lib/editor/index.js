  ...
  listenForCurrentLine() {
    this.subscriptions.add(this.textEditor.observeCursors((cursor) => {
      ...
      const handlePositionChange = ({ start, end }) => {
        ...
        let currentRange = Range.fromObject([[start.row, 0], [end.row, Infinity]])

        if (lastRange && lastRange.isEqual(currentRange)) return
        if (marker) marker.destroy()
        lastRange = currentRange
        // NOTE: Atom does not paint gutter if multi-line and last line
        // has zero index
        if (start.row !== end.row && Range.fromObject([start, end]).end.column === 0) {
          // ^ We need that Range.fromObject hack because when we focus index 0 on multi-line selection
          ...
          currentRange = Range.fromObject(currentRange)
          currentRange.end.row--
        }

        marker = this.textEditor.markBufferRange(currentRange, {
          invalidate: 'never',
          maintainHistory: false,
        })
        const item = document.createElement('span')
        item.className = `line-number cursor-line linter-cursor-line ${start.row === end.row ? 'cursor-line-no-selection' : ''}`
