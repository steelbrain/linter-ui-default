  ...
  updateMessages({added, removed}) {
    if (this.highlight) {
      ...
      for (let i = 0; i < addedLength; ++i) {
        ...
        const messageRow = messageRange.start.row
        ...
        const marker = this.editor.markBufferRange(messageRange, {invalidate: 'never'})
        const gutter = document.createElement('span')
        gutter.className = `linter-gutter linter-highlight ${messageClass}`
