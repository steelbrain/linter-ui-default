...
export class Editor {
  ...
  updateBubble(position) {
    let message = null
    // Todo: Use markers from gutters, as the markers stored in messages become outdated

    if (this.bubbleMessage && this.bubbleMessage.range.containsPoint(position) &&
        this.rowMessages[this.bubbleMessage.range.start.row].indexOf(this.bubbleMessage) !== -1
    ) {
      ...
      if (rowMessages) {
        ...
        for (let i = 0; i < rowMessagesLength; ++i) {
          ...
          if (currentMessage.range.containsPoint(position)) {
    ...
    if (message !== null) {
      ...
      const marker = this.editor.markBufferRange([position, position], {invalidate: 'inside'})
      this.editor.decorateMarker(marker, {
        type: 'overlay',
        item: bubbleElement
      })
