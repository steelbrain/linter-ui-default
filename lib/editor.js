...
export default class Editor {
  ...
  emitter: Emitter;
  markers: Map<Message, BufferMarker>;
  ...
  constructor(textEditor: TextEditor) {
    ...
    this.subscriptions.add(atom.config.observe('linter-ui-default.showCopyButton', showCopyButton => {
  ...
  updateBubble(position: ?Point = null) {
    ...
    if (messages.size) {
      this.bubbleRange = messages.size === 1 ? Array.from(messages)[0].location.position : null
      ...
      this.textEditor.decorateMarker(this.bubble, {
        type: 'overlay',
        persistent: false,
        item: getBubbleElement(messages, this.showProviderName, this.showCopyButton),
      })
    }
  }
  removeBubble() {
  ...
  apply(added: Array<Message> | Set<Message>, removed: Array<Message> | Set<Message>) {
    ...
    for (const message of added) {
      const marker = textBuffer.markRange(message.location.position, {
        invalidate: 'never',
        persistent: false,
      })
