...
import type { Message, Config$ShowIssues } from './types'
...
export default class Panel {
  ...
  apply(messages: ?Array<Message> = null) {
    if (messages) {
    ...
    if (this.showIssuesFrom === 'All Files') {
      ...
      filteredMessages = filter(messages, message => message.location.file === activePath)
      ...
      if (activeEditor) {
        ...
        filteredMessages = filter(messages, message => message.location.file === activePath && editorRange.intersectsWith(message.location.position))
