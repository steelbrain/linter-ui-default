...
export class Panel {
  constructor() {
    ...
    this.filter = atom.config.get('linter-ui-default.messageFilter')
    ...
    this.subscriptions.add(atom.workspace.onDidChangeActivePaneItem(paneItem => {
      ...
      if (this.visibility && this.filter !== ENTIRE_PROJECT) {
        this.reRenderMessages()
      }
    }))
    this.subscriptions.add(atom.config.onDidChange('linter-ui-default.messageFilter', ({newValue}) => {
  ...
  updateMessages({added, removed, messages}) {
    ...
    for (let i = 0; i < addedLength; ++i) {
      ...
      if (this.filter === ENTIRE_PROJECT || (
          this.filter === CURRENT_FILE && activePath === message.filePath
        )) {
