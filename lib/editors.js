...
import type { Message, MessagesPatch, Config$ShowIssues } from './types'
...
export default class Editors {
  ...
  apply(difference: MessagesPatch, force: boolean = false) {
    ...
    for (const message of difference.added) {
      if (editorsMap[message.location.file]) {
        editorsMap[message.location.file].added.push(message)
      }
    }
    for (const message of difference.removed) {
      if (editorsMap[message.location.file]) {
        editorsMap[message.location.file].removed.push(message)
      }
    }
  ...
  filterAndApply(editor: Editor) {
    ...
    for (const message of this.messages) {
      if (message.location.file === editorPath) {
