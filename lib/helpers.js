...
export function visitMessage(message: LinterMessage, reference: boolean = false) {
  ...
  let messagePosition
  if (reference) {
    invariant(message.version === 2, 'Only messages v2 are allowed in jump to reference')
    invariant(message.reference && message.reference.file, 'Message.reference must be valid')
